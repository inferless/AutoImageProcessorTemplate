...
class InferlessPythonModel:
    def initialize(self):
        ...
        self.model = AutoModel.from_pretrained("histai/hibou-L", trust_remote_code=True).to("cuda")

    def infer(self, inputs: RequestObjects) -> ResponseObjects:
        response = requests.get(inputs.image_url)
        image = Image.open(BytesIO(response.content))

        model_inputs = self.processor(images=image, return_tensors="pt").to("cuda")
        model_outputs = self.model(**model_inputs)
        output_list = model_outputs.pooler_output.tolist()
